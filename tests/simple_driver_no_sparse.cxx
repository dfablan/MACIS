...
#define OPT_KEYWORD(STR, RES, DTYPE) \
  ...
  if(ci_exp == CIExpansion::CAS) {
    ...
    E0 = macis::CASRDMFunctor<generator_t>::rdms(
        ...
        V_active.data(), active_ordm.data(), active_trdm.data(),
        C_local MACIS_MPI_CODE(, MPI_COMM_WORLD));
    ...
    ham_gen.form_rdms(
        dets.begin(), dets.begin(), dets.end(), dets.end(), C.data(),
  ...
  if(testGF) {
    ...
    for(int i = 0; i < n_imp; i++)
      std::cout << "GF_orbs_basis[" << i
                << "] = " << gf_settings.GF_orbs_basis[i] << std::endl;
    for(int i = 0; i < n_imp; i++)
      std::cout << "GF_orbs_comp[" << i << "] = " << gf_settings.GF_orbs_comp[i]
                << std::endl;
    for(int i = 0; i < n_imp; i++)
      std::cout << "is_up_basis[" << i << "] = " << gf_settings.is_up_basis[i]
                << std::endl;
    for(int i = 0; i < n_imp; i++)
      std::cout << "is_up_comp[" << i << "] = " << gf_settings.is_up_comp[i]
                << std::endl;
    ...
    std::vector<double> occs(n_active);
