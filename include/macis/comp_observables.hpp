...
auto comp_observables(void* params, bool db_occs_flag, bool sz_sz_flag,
                      bool tz_tz_flag) {
  struct impurity_params* p = static_cast<impurity_params*>(params);
      ...
      for(size_t site_j = 0, site_j < n_sites; site_j++) {
        for(size_t band_i = 0; band_i < n_bands; band_i++) {
          for(size_t band_j = 0; band_j < n_bands; band_j++) {
            ...
            sz_sz[site_i + site_j * n_sites] +=
                0.25 * (trdm_uu[a + a * n_imp + b * n_imp2 + b * n_imp3] -
        ...
        for(size_t band_i = 0; band_i < n_bands; band_i++) {
          for(size_t band_j = 0; band_j < n_bands; band_j++) {
            ...
            if(band_i == band_j)
              ...
              sign = -1.0;
            tz_tz[site_i + site_j * n_sites] +=
                0.25 *
                (sign * trdm_uu[a + a * n_imp + b * n_imp2 + b * n_imp3] +
