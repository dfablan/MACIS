...
#pragma omp parallel for
    ...
    if(it + nbands <= nLanIts) {
      ...
      if(std::abs(bandH[it - 1][it + nbands - 1]) < thres) {
        //          std::cout << "BAND LANCZOS STOPPED PREMATURELY DUE TO SMALL
        //          NORM! NAMELY " << bandH[it-1][it + nbands-1] << ", STOPPED
        //          AT ITERATION: " << it << std::endl; nLanIts = it; for(int i
        //          = 0; i < nLanIts; i++) bandH[i].resize(nLanIts);
