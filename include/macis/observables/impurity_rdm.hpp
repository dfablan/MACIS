...
template <size_t N>
ImpBathDecomp<N> decompose_det(const macis::wfn_t<N>& det,
                               size_t n_imp,
                               size_t n_active) {
  ImpBathDecomp<N> out;
...
inline double determinant_local_implementation(std::vector<double> mat, int n) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (std::abs(mat[(size_t)i*n+i]) < 1e-300) return 0.0; // Matrix is singular
...
inline int compute_fermionic_sign(int n_imp_up_bra, int n_imp_down_bra,
    ...
    phase += 1LL * n_bath_down * (n_imp_up_bra + n_imp_down_bra + n_imp_up_ket + n_imp_down_ket);
    return (phase & 1LL) ? -1 : +1;
}

inline std::vector <double> compute_overlap_matrix(const std::vector<double>& rot_mat, int n_imp_orbitals, int n_active_orbitals, const std::vector<uint64_t>& basis_states) {

    const int n_bits = 2 * n_imp_orbitals;

    if (n_bits > 64) {
        throw std::runtime_error("Error: 2*n_imp_orbitals > 64, cannot compute overlap matrix with uint64_t representation");
    }
    ...
    rot_mat_restricted_up.reserve((size_t)n_imp_orbitals * (size_t)n_imp_orbitals);
    rot_mat_restricted_down.reserve((size_t)n_imp_orbitals * (size_t)n_imp_orbitals);

    for (size_t i = 0; i < basis_size; ++i) {
        const uint64_t bra_state = basis_states[i];
        const uint64_t mask = (1ULL << (size_t)n_imp_orbitals) - 1;
        // extract the first n_imp_orbitals bits for up and the next n_imp_orbitals bits for down (from left to right)
        ...
        for (size_t j = 0; j < basis_size; ++j) {
            ...
            //select only the states with the same number of occupied orbitals in the bra and ket states
                ...
                if ((bra_up >> (size_t)(n_imp_orbitals - 1 - m)) & 1ULL) {
                    for (int n = 0; n < n_imp_orbitals; ++n) {
                        if ((ket_up >> (size_t)(n_imp_orbitals - 1 - n)) & 1ULL) {
                            rot_mat_restricted_up.push_back(rot_mat[(size_t)m * (size_t)n_active_orbitals + (size_t)n]);
                        }
                    }
                }
                if ((bra_down >> (size_t)(n_imp_orbitals - 1 - m)) & 1ULL) {
                    for (int n = 0; n < n_imp_orbitals; ++n) {
                        if ((ket_down >> (size_t)(n_imp_orbitals - 1 - n)) & 1ULL) {
                            rot_mat_restricted_down.push_back(rot_mat[(size_t)m * (size_t)n_active_orbitals + (size_t)n]);
                        }
                    }
                }
            }
            ...
            overlap_mat[i * basis_size + j] =
                determinant_local_implementation(rot_mat_restricted_up,   n_occ_bra_up) *
                determinant_local_implementation(rot_mat_restricted_down, n_occ_bra_down);
        }
    }
...
template <size_t N>
std::vector<double> build_reduced_density_matrix(const std::vector<ImpBathDecomp<N>>& determinants,  const std::vector<double>& C, const std::vector<uint64_t>& basis_states, const std::vector<double>& overlap_matrix) {
            ...
            try {
                ...
                for (size_t idx : group_indices) {
                    ...
                    if (bidx < basis_size) {
                        ...
                        occs_vector[bidx] = {
                            det.n_imp_up, det.n_imp_dn,
                            det.n_bath_up, det.n_bath_dn
                        };
                    }
                }
                ...
                for (size_t i = 0; i < basis_size; ++i) {
                    ...
                    for (size_t j = 0; j < basis_size; ++j) {
                        ...
                        const int sgn = compute_fermionic_sign(
                            occs_vector[i][0], occs_vector[i][1],
                            occs_vector[j][0], occs_vector[j][1],
                            occs_vector[i][2], occs_vector[i][3]
                        );
...
std::vector<double> compute_impurity_rdm_from_state(
    ...
    const std::vector<double>& orb_rot) {

  if (dets.size() != C.size()) {
    throw std::runtime_error("compute_impurity_rdm_from_state: dets/C size mismatch");
  }
  if (n_imp > 32) {
    throw std::runtime_error("compute_impurity_rdm_from_state: current uint64_t packing assumes n_imp <= 32 per spin block");
  }
  ...
  overlap_matrix = compute_overlap_matrix(orb_rot, n_imp, n_active, basis_states);
  ...
  rho = build_reduced_density_matrix(decomposed_dets, C, basis_states, overlap_matrix);
