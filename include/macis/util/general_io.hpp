 ...
 * @param is_column_major Whether the matrix is stored in column-major order (default: true)
 * @param width Field width for each element (default: 12)
 * @param output Output stream (default: std::cout)
 */
template<typename T>
void write_matrix(const T* mat, int rows, int cols, 
 ...
 */
template<typename T>
void write_vector(const T* vec, int size,
                  const std::string& filename,
                  int width = 12) {
